...
class JSON_IMAS:
    ...
    def readJSON(self, filename, time):
        """
        ...
        d['bcentr'] = data['equilibrium']['vacuum_toroidal_field']['b0'][tIdx] * -1.0
        ...
        d['siAxis'] = eqt['global_quantities']['psi_axis'] * -1.0 / (2.0*np.pi)
        d['siBry'] = eqt['global_quantities']['psi_boundary'] * -1.0 / (2.0*np.pi)
        ...
        if len(d['fpol']) != d['R1D']:
            d['fpol'] = np.interp(psiN, np.linspace(0,1,len(d['fpol'])), d['fpol'])
        ...
        #2D profiles
        d['psirz'] = np.array(eqt['profiles_2d'][0]['psi']).T * -1.0 / (2.0*np.pi)
        ...
        d['R1'] = d['Rmin']
        d['Zmid'] = 0.0
        d['Ip'] = eqt['global_quantities']['ip'] * -1.0
        d['thetapnts'] = 2*d['nw']
