...
class JSON_IMAS:
	...
	def getDeuterium(self, profiles = None, listAll = False):
		"""
		Get the index for the Deuterium species
		Needs self.profiles variable, which is generated by self.coreProfiles()
		listAll: bool, optional, show all available ion species
		"""
		if profiles is None: profiles = self.profiles
		foundIt = False
		if listAll: print('Ion species available: ', profiles['ions'])
		for idx, ion in enumerate(profiles['ions']):
	...
	def coreProfiles(self, time, dx = 0.0025, xmin = 0.7, xmax = 1.2, nsol = 0.02, Tsol = 5e-3, 
	...
	def sanityCheck(self, time, m3dc1 = True, returnData = False):
		...
		dIons = {'ions':[profiles['ion'][item]['label'] for item in range(len(profiles['ion']))]}
		idx,ion = self.getDeuterium(dIons, listAll = True)
	...
	def plotProfile(self, what = 'all', fig = None, c = None, label = '', extended = False, style = None):
		...
		if what in ['p','P','Pres','pres','pressure','Ptot','ptot','Press','press','Pressure']:
			...
			ylabel = ion + ', n$_i$ [10$^{20}$/m$^{3}$]'
			y = profiles[ion]['ni']*1e-20
		elif what in ['ti','Ti','iontemperature']:
			ylabel = ion + ", T$_{i}$ [keV]"
			y = profiles[ion]['Ti']*1e-3
		...
		if what in ['all']:
			...
			ax5.set_ylabel(ion + ', n$_i$ [10$^{20}$/m$^{3}$]')
			y = profiles[ion]['ni']*1e-20
			ax5.set_xlim(0,x.max())
			ax5.get_xaxis().set_ticklabels([])
			if extended: 
				ax5.plot(x, y, style, color = 'k', lw = 2)
				ax5.plot(self.profiles['psi'], self.profiles[ion]['ni']*1e-20, style, color = 'r', lw = 2)
			else: ax5.plot(x, y, '-', color = c, lw = 2)
			ax5.set_ylim(bottom=0)
			
			ax6 = fig.add_subplot(326, aspect = 'auto')
			ax6.set_ylabel(ion + ", T$_{i}$ [keV]")
			y = profiles[ion]['Ti']*1e-3
			ax6.set_xlim(0,x.max())
			ax6.set_xlabel('$\\psi$')
			if extended: 
				ax6.plot(x, y, style, color = 'k', lw = 2)
				ax6.plot(self.profiles['psi'], self.profiles[ion]['Ti']*1e-3, style, color = 'r', lw = 2)
	...
	def writeGEQDSK(self, file, shot = None, time = None):
		...
		with open(file, 'w') as f:
			...
			f.write('% .9E% .9E% .9E% .9E% .9E\n'%(self.eqd['zmaxis'],0,self.eqd['siBry'],0,0))
